...
class BaseAthenaBucket(s3.Bucket):
    ...
    def __init__(self, scope: core.Construct, deploy_env: Environment, **kwargs) -> None:
        ...
        self.obj_name = f's3-juan-armond-{self.deploy_env.value}-data-lake-athena-results'
...
class BaseAthenaWorkgroup(athena.CfnWorkGroup):
    ...
    def __init__(self, scope: core.Construct, deploy_env: Environment,
                 athena_bucket: BaseAthenaBucket, gb_scanned_cutoff_per_query: int, **kwargs) -> None:
        self.gb_scanned_cutoff_per_query = gb_scanned_cutoff_per_query
        self.deploy_env = deploy_env
        self.athena_bucket = athena_bucket
        self.obj_name = f's3-juan-armond-{self.deploy_env.value}-data-lake-athena-workgroup'
        super().__init__(
            scope,
            id=self.obj_name,
            name=self.obj_name,
